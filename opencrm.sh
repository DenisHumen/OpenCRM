...
# Секрет приёма тревог: одна переменная на приложение и на Alertmanager.
#
# Приём был закрыт только тем, что порт 8000 наружу не публикуется. Соседа по
# сети compose это не закрывало — он мог слать ложные тревоги от имени системы
# или топить настоящие шумом.
#
# Заводится молча и один раз: спрашивать тут нечего, а перевыдача сломала бы
# доставку до перезапуска обоих контейнеров.
configure_alerts_secret() {
    if [ -z "$(env_get "$DOCKER_ENV" OPENCRM_ALERTS_SECRET 2>/dev/null || true)" ]; then
        env_set "$DOCKER_ENV" OPENCRM_ALERTS_SECRET "$(gen_secret 32)"
    fi
}
