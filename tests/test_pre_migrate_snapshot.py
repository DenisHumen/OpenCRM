...
то есть только для файловой базы. На установке, работающей на MySQL, в журнале
запуска было видно старую копию файла и следом миграции по MySQL: ни
`mysqldump`, ни новых копий не было вовсе. Неудачное обновление откатывало код
и оставляло базу такой, какой её сделала полупрошедшая миграция.

Здесь проверяется точка входа целиком — настоящим `sh`, с подставным `python`.
Двойников для оболочки не бывает: беда в этом файле будет не логической, а
шелловой (кавычка, `set -e` на ровном месте, `if`, закрывший не ту ветку), и
видно её только настоящему интерпретатору.
"""
...
def _stend(tmp_path: Path, *, db_url: str = MYSQL_URL, golova: str = GOLOVA) -> dict:
    """Каталог с подставным python и следом его вызовов."""
...
def test_povtornyy_start_ne_zatiraet_kopiyu(tmp_path):
    """Копия называется по ревизии, с которой уходим, и по одной на ревизию.
