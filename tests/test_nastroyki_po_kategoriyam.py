...
BEZ_PUNKTA: dict[str, str] = {
    ...
    # Приём заявок и ключи доступа слиты в один экран `/settings/api` (владелец,
    # 07.09.2026: «почти одно и то же»). Прежние два адреса оставлены
    # перенаправлениями: они разосланы в руководстве, в виджете ключа на сводке
    # и в чужих закладках, и «страница не найдена» по такой ссылке выглядит
    # поломкой системы, а не переездом раздела.
    "/settings/leads": "перенаправление на /settings/api, своего экрана больше нет",
    "/settings/api-keys": "перенаправление на /settings/api, своего экрана больше нет",
}
