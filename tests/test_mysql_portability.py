"""Схема обязана строиться не только в SQLite, но и в MySQL.

Проект живёт на SQLite, а вырасти должен в MySQL. Беда переносимости в том,
что она **не проверяется тем, чем проверяется всё остальное**: тесты гоняют
SQLite, там всё зелено, а несовместимость всплывает через полгода — на живой
базе, посреди переезда, когда откатываться уже некуда.
...
русском не помещается там, где помещалось. Ни одно из этих расхождений себя не
объявляет.

Проверки ниже механические нарочно: живого MySQL в наборе тестов нет, поэтому
сверяем то, что можно сверить без сервера, — какой DDL SQLAlchemy **соберёт**
для диалекта mysql. Это ловит и то, и другое, и стоит миллисекунды.
...
DLINNYE = {("mail_messages", "body_text"), ("mail_messages", "body_html")}
...
def test_u_tekstovyh_kolonok_default_zapisan_vyrazheniem():
    ...
    принимают оба движка (проверено на MySQL 8.0.46 и SQLite).
    """
    guilty = []
