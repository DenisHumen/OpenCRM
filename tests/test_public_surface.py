...
PUBLIC_ROUTES = {
    ...
    ("GET", "/b/{token}"),
    ("POST", "/b/{token}/pin"),
    # Гость называет часовой пояс: сессии у него нет, доска проверяется по
    # токену ссылки, тумблер доски гасит ручку отказом, поток ограничен.
    ("GET", "/b/{token}/data"),
    ("GET", "/media/{work_uid}/{filename}"),
    # состояние заказа по QR с квитанции: ограничитель по адресу, 20 за 10 минут
    ("GET", "/d/{number}"),
    # Файл по ссылке: за него отвечает токен, срок, код и режим. Байты идут той
    # же дорогой, что и страница, — отдельного адреса у файла не существует, и
    # это записано отдельно потому, что у витрин однажды было наоборот:
    # страница отвечала 401, а картинки 200 (`share_service.media_is_public`).
    ("GET", "/f/{token}"),
    ("POST", "/f/{token}/pin"),
    ("GET", "/f/{token}/download"),
    # Байты на показ: тот же токен, срок, код и блок, плюс подписанный ключ на
    # десять минут. Он и есть защита просмотра — скопированный со страницы
    # адрес картинки перестаёт работать раньше, чем дойдёт до получателя.
    ("GET", "/f/{token}/view"),
    # Гость называет почту по ссылке для приглашённых: сессии у него нет, а
    # адрес закрыт списком, счётчиком попыток (тем же, что у кода) и тем, что
    # отказ не отличается от «набрали криво» — иначе список отдавался бы по
    # одному адресу за попытку.
    ("POST", "/f/{token}/guest"),
    ...
    # приём от телеграма: секрет в заголовке вместо сессии, сверяется сравнением
    # постоянного времени, перебор притормаживается ограничителем до сверки
    ("POST", f"{API}/telegram/webhook"),
    # тревоги от Alertmanager: сессии у него нет, вместо неё граница сети —
