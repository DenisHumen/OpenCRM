...
#:
#: Все девять ждут ОДНОГО решения владельца: правка `AREA_ACTIONS` меняет
#: редактор должностей и осиротит строки `role_permissions` на работающих
#: серверах, поэтому делается разом и с миграцией. Развилка записана в
#: `docs/20-udobstvo-i-spravka.md` §2.1.
#:
#: Список закрытый: новое пустое право сюда не дописывается «чтобы позеленело».
PUSTYE_NAROCHNO: dict[str, str] = {
    "documents.delete": "бланк не удаляется, а отменяется — правило учёта",
    "orders.delete": "то же: выданная бумага не исчезает",
    "waybills.delete": "то же; отмечено комментарием в core/permissions.py",
    "mail.delete": "почта — зеркало сервера, письмо не стирают",
    "mail.edit": "правка письма задним числом подделывала бы переписку",
    "telephony.delete": "журнал звонков только дописывается",
    "monitoring.create": "раздел только показывает состояние, заводить нечего",
    "monitoring.delete": "то же",
    "monitoring.edit": "то же",
}
