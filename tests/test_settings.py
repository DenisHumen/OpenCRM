...
def test_healthz():
    ...
    #
    # `redis` — состояние общего счётчика попыток: ok / down / off. Кода ответа
    # оно НЕ меняет, и это решение: на 200 здесь завязаны проверка здоровья
    # docker и откат обновления, а они умеют одно — заменить контейнер
    # приложения. Лежащий Redis этим не чинится, зато откат исправного
    # обновления из-за соседней службы стоит простоя на ровном месте.
