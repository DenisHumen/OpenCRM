...
def test_zayavka_nakhoditsya_po_imeni_klienta_bez_join(manager_client, db):
    ...
    заставляло планировщик вести запрос ОТ клиентов: 200 000 проходов и на
    каждый поиск в `ix_deals_client_id`. Замерено на большой базе: 2457 → 440 мс
    после замены на подзапрос, найденное совпало до строки.
