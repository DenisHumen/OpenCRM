...
def _script() -> str:
    ...
    гоняются и там (автообновление поднимает набор в образе `--target tests`). Красный
...
def test_provalennoe_perechityvanie_vidno_v_uvedomlenii():
    ...
    # Здесь проверяется только то, что причина ДОЕХАЛА до шага. Что она потом
    # попадает в уведомление, стережётся поведением, а не текстом исходника:
    # tests/test_autoupdate.py, `test_a_failed_reload_is_named_in_the_message_
    # even_when_the_update_worked` — оно рендерит сообщение и ищет в нём причину.
    # Проверка по именам переменных ломалась от переименования и ничего при этом
    # не гарантировала.
    assert "upali" in kod or "failed" in kod, (
        "в уведомлении не осталось разбора провалившихся шагов"
    )
