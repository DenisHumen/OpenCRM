...
EXEMPT: dict[str, str] = {
    ...
    "/dashboard/layout": "своя раскладка сводки — не раздел системы; право и блок каждого виджета проверяет vidzhety_service",
    ...
    "/search/{area}": (
        "продолжение группы поиска: тот же отбор внутри, теми же проверками "
        "(`_group` — сначала блок, потом право), что и первая страница. Одного "
    ...
    "/notifications": (
        "свои уведомления: закрыты входом (`get_current_user`), а не правом — "
        "адресат уже отобран правами при записи (`notification_service.adresaty`), "
        "и чужого запрос не отдаёт по условию `user_id`"
    ),
    "/notifications/summary": "счётчик своих уведомлений — то же, что и список",
    "/notifications/read": "отметка о прочтении только своих — то же, что и список",
    "/live": (
    ...
    "/system/github": "звёзды проекта: украшение панели, которую видят все сотрудники",
}
...
WRITE_BY_VIEW_OK: dict[str, str] = {
    ...
    "/keys/{key_id}/code": (
        "показать код — это ЧТЕНИЕ ключа, в нём ничего не меняется. POST здесь "
        "не ради правки, а ради журнала и кэша: показ пишется отдельной "
        "записью, а GET кэшируют и браузер, и прокси — код доехал бы до "
        "второго смотрящего из кэша, минуя и права, и журнал"
    ),
    "/keys/{key_id}/secret": (
        "тот же довод, и вдобавок вернее прочих: секрет отдаётся только root "
        "(`klyuchi_service.sekret`), и право на раздел здесь ничего не решает "
        "— решает роль. Право `edit` на этой ручке означало бы, что секрет "
        "видит всякий, кому дали править названия"
    ),
}
