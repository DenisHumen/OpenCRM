...
RAZRESHENO: dict[str, str] = {
    ...
    "web/api/schemas.py:client_out": (
        "доли градуса у точки клиента на глобусе — не деньги: широта 50.45 "
        "это широта, а не полтинник. Целыми они лежат в базе (1e-7 градуса), "
        "дробь появляется только в ответе экрану (06.09.2026)"
    ),
    "core/services/report_service.py:share": (
        "доля в процентах — отношение двух счётчиков (конверсия, доля возвратов), "
        "а не деньги; одна цифра после запятой нужна подписи на экране. Файл попал "
        "под сторож с отчётом о долгах (06.09.2026), где деньги целые"
    ),
}
