...
def _pokazat(dom: pathlib.Path, klavishi: str = "", sekund: float = 7.0) -> str:
    ...
    kody = {
        ...
        "enter": b"\r", "q": b"q",
        # Shift+стрелка и F5: незнакомые последовательности, чей хвост
        # раньше дочитывался как отдельные нажатия.
        "shift_down": b"\x1b[1;2B", "f5": b"\x1b[15~",
    }
    vyvod = b""
    nachalo = time.time()
    poslano = False
...
@nuzhen_pty
def test_hvost_neznakomoy_posledovatelnosti_nichego_ne_nazhimaet(tmp_path):
    """Shift+стрелка и F5 не должны нажимать пункты меню. Раньше — нажимали.

    Разбор бросал незнакомую последовательность, не дочитав её до конечного
    байта. Остаток лежал в буфере терминала и читался дальше как отдельные
    нажатия. У `ESC [ 1 ; 2 B` (Shift+Down) в остатке `;`, `2`, `B`, а цифра в
    меню не переводит подсветку — она ставит `enter` и ТУТ ЖЕ исполняет пункт.

    Замерено подрывом: Shift+Down открывал раздел «Управление», а следом хвост
    F5 (`1`, `5`, `~`) нажимал в нём пятый пункт — «Назад». Конечный экран от
    этого выглядел совершенно невинно: то же главное меню. Отличие ровно одно —
    выбранным оказывался ЧУЖОЙ пункт. Поэтому проверка смотрит на пояснение
    внизу: оно принадлежит выбранному пункту и соврать не может.

    В разделе «Копии» вторым пунктом стоит восстановление из копии.
    """
    ekran = _ekran(_pokazat(_stend(tmp_path), "shift_down,f5"))
    vidno = chr(10).join(s for s in ekran if s.strip())

    assert "Что сейчас с сайтом" in vidno, (
        "выбор уехал с первого пункта — хвост последовательности нажал за человека:"
        + chr(10) + vidno
    )
    assert "Запуск, перезапуск, остановка" not in vidno, (
        "выбран пункт, которого никто не выбирал:" + chr(10) + vidno
    )
