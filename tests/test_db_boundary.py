...
def _sources():
    # `nagruzka` в списке НЕ потому, что там ожидается продуктовый код, а
    # наоборот: каталог верхнего уровня, который проверка не обходит, — слепое
    # пятно. Сегодня в нём стенд, завтра туда переедет что-нибудь живое, и
    # граница промолчит. Пусть лучше нарушение будет названо в `ALLOWED`.
    for area in ("core", "web", "config", "scripts", "deploy", "nagruzka"):
