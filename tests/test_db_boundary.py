...
ALLOWED = {
    ...
    # сброс root'а чинит базу, в которую иначе не войти, а уборка и пересчёт
    # медиа ходят по всей таблице разом.
    "scripts/reset_root.py",
    "scripts/reprocess_media.py",
    "scripts/purge_deleted.py",
    # Проверка резервной копии открывает ЧУЖОЙ дамп — снятый месяц назад, с
    # прежней схемой, возможно оборванный. Репозитории здесь не годятся вовсе:
    # они знают сегодняшние модели и ходят в живую базу, а тут читается текст
    # копии, которую как раз и проверяют на годность.
