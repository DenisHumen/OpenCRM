...
def test_prosba_ubrat_dvizhenie_ubiraet_i_ozhidanie():
    """Обнулять одну длительность мало: остаётся задержка.

    Каскад появления плиток на витрине задаётся именно задержкой — до 900 мс
    (`showcase.html`). С одной обнулённой длительностью человек, попросивший
    систему убрать анимацию, всё равно ждал почти секунду перед пустым местом,
    а потом плитки возникали разом: и движения нет, и ожидание есть — худший
    вариант из двух.

    Счётчик повторов — про бесконечные анимации: пульсирующая точка страницы
    обслуживания иначе мигала бы вечно, просто быстро. Там это учтено давно, в
    общем шаблоне правило не появилось.
    """
    base = _read(ROOT / "web" / "public" / "templates" / "base.html")
    blok = base[base.index("prefers-reduced-motion: reduce"):]
    blok = blok[: blok.index("}")]
    for svoystvo in ("animation-delay", "animation-iteration-count", "transition-delay"):
