...
def _duel(strike, first, second):
    ...
    **Единственное место во всём наборе, где два запроса идут по-настоящему
    ...
    Неожиданный отказ базы прилетает сюда именно исключением, а не
    ответом: общего обработчика для таких ошибок у приложения нет
...
def test_two_owners_demoting_each_other_leave_one_standing(root_client):
    ...
    try:
        ...
        # чем кончилась гонка.
        #
        # «Прошёл ровно один» тестом быть не может: под двумя одновременными
        # записями база вправе отказать ОБОИМ — вместо нашего 403 прилетит
        # отказ по замку. Владельцев тогда остаётся двое, и это правильный
        # исход: инвариант «владелец есть» цел. Требовать непременно одного
        # значит требовать, чтобы гонку всегда кто-то выигрывал, — а этого
        # никто не обещал, и тест начинает мигать.
        survivors = _root_ids()
