...
POST_WITHOUT_LATCH: dict[tuple[str, str], str] = {
    ...
    ("Kolokolchik.tsx", "/notifications/read"): (
        "отметка «прочитано»: записи не заводит, второй вызов с теми же номерами "
        "ставит ту же отметку в то же самое значение"
    ),
...
def test_potok_sobytiy_otkryvaetsya_v_odnom_meste():
    ...
    # работа (docs/ustroystvo/12-zhivye-obnovleniya.md §6); пока правило прежнее: на назначение —
...
def test_kolco_fokusa_est_u_vsego_klavishnogo():
    ...
    кнопок и карточек-ссылок не менялось ничего. `docs/dizayn/05-dizayn-crm.md` называл
...
# --- список не кончается там, где кончилась страница ------------------------
#
# Болезнь, найденная разом на полутора десятках экранов: запрос просил
# `per_page=100`, экран показывал сотню записей и рядом честно писал «всего
# 3400». То есть сам сообщал, что показывает тридцатую часть, и не давал ничего
# с этим сделать. Ни отказа, ни пустоты, ни вертушки — ровный, уверенный,
# неполный список.
#
# Каждое место здесь либо умеет спросить следующую страницу, либо названо ниже
# вместе с причиной, почему предел ему по делу.

# Ключ — файл и кусок строки запроса; значение — довод.
PREDEL_BEZ_STRANITS: dict[tuple[str, str], str] = {
    # Подсказки по мере набора. Восемь строк — это не «первые восемь из
    # тысячи», а столько, сколько влезает в выпадашку: набрал ещё букву —
    # отбор сузился. Листать подсказки незачем и некуда.
    ("VyborKlienta.tsx", "per_page=8"): "подсказка выбора клиента, сужается набором",
    ("DocumentCard.tsx", "per_page=8"): "подсказка выбора товара, сужается набором",
    ("OrderCard.tsx", "per_page=8"): "подсказка выбора товара, сужается набором",
    ("DealLines.tsx", "per_page=8"): "подсказка выбора товара, сужается набором",
...
def test_spisok_ne_konchaetsya_na_pervoy_stranitse():
    """Запрос с пределом обязан оставлять человеку способ спросить продолжение.

    Способа два, и `page` в коде — не один из них: страница, которую негде
    попросить, для человека такой же потолок, как её отсутствие. Значит либо в
    файле есть кнопка `<Dochitat>`, либо запрос идёт через `useReference` (тот
    собирает справочник по страницам целиком — набор, из которого выбирают,
    половиной показывать нельзя). Всё остальное называется в списке выше вместе
    с доводом.

    Ищем по слову `per_page`, а не по числу возле него: числа давно уехали в
    постоянные (`per_page: String(NA_STRANITSE)`), и проверка, смотревшая на
    цифры, перестала видеть ровно те экраны, ради которых заведена.
    """
    naydeno = 0
    potolki = []
    for path in sorted(SCREENS.rglob("*.tsx")):
        text = path.read_text(encoding="utf-8")
        smeshchenie = 0
        for nomer, stroka in enumerate(text.splitlines(True), 1):
            nachalo, smeshchenie = smeshchenie, smeshchenie + len(stroka)
            if "per_page" not in stroka:
                continue
            naydeno += 1
            szhato = re.sub(r"[\s\"'{}$]", "", stroka)
            if any(
                imya == path.name and kusok.replace(" ", "") in szhato
                for imya, kusok in PREDEL_BEZ_STRANITS
            ):
                continue
            # `useReference` дочитывает страницы сам — предел в его пути это
            # размер страницы, а не потолок показа. Смотрим рядом: справочник
            # и его путь пишутся одной строкой.
            if "useReference" in text[max(0, nachalo - 200):nachalo + 200]:
                continue
            # Кнопка «показать ещё» — и есть «продолжение спросить есть чем».
            # Ищем по всему файлу, а не рядом: отбор собирают в `useMemo`
            # наверху, а страницу просят в загрузке строк на сорок ниже, и
            # проверка по окрестности врала бы на каждом втором экране.
            if "<Dochitat" in text:
                continue
            potolki.append(f"{path.name}:{nomer}: {stroka.strip()}")

    assert naydeno >= 15, f"мест с пределом найдено {naydeno} — проверка смотрит не туда"
    assert not potolki, (
        "список обрывается на первой странице, и спросить продолжение нечем:\n  "
        + "\n  ".join(potolki)
    )


#: Чем сторожат опоздавший ответ дочитки. Имя — часть уговора, ровно как у
#: засова `useGuard`: сторож ищет приём по имени, потому что увидеть его иначе
#: (что сверка стоит МЕЖДУ ответом и записью в состояние) чтением текста нельзя.
STOROZHA_DOCHITKI = ("otbor_spiska.current !==", "pokolenie.current !==", "sproshen_otbor.current !==")


def test_dochitka_otbrasyvaet_opozdavshiy_otvet():
    """Страница, приехавшая после смены отбора, в список не попадает.

    Порядок событий, из-за которого правило заведено:

      1. человек нажал «показать ещё» — ушла вторая страница по «иван»;
      2. пока она едет, он набрал «п» — загрузка запросила первую страницу «п»;
      3. первая страница «п» приехала и ЗАМЕНИЛА список;
      4. приехала вторая страница «иван» и ДОПИСАЛАСЬ к находкам по «п».

    На экране два отбора вперемешку, а «всего» — от прошлого. Заметить нечем:
    список выглядит обычным, отказа нет, пустоты нет.

    Флажок `let current = true` эту беду не ловит и ловить не может: он живёт в
    уборке ЭФФЕКТА и снимается, когда эффект перезапускается, а дочитка — не
    эффект, у неё уборки нет вовсе. Нужна отметка, которую ставит загрузка и
    сверяет дочитка.

    Там, где отбора нет вовсе (доска, канбан), сторожат поколение списка: он
    перечитывается целиком, и страница прошлого списка так же не должна
    дописаться к новому.
    """
    s_dochitkoy = []
    bezzashchitnye = []
    for path in sorted(SCREENS.rglob("*.tsx")):
        text = path.read_text(encoding="utf-8")
        if "<Dochitat" not in text:
            continue
        s_dochitkoy.append(path.name)
        if not any(storozh in text for storozh in STOROZHA_DOCHITKI):
            bezzashchitnye.append(path.name)

    assert len(s_dochitkoy) >= 10, (
        f"экранов с дочиткой найдено {len(s_dochitkoy)} — проверка смотрит не туда"
    )
    assert not bezzashchitnye, (
