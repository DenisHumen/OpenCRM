...
def switch_all(client, enabled: bool) -> None:
    ...
    При выключении порядок обратный: каскад гасит надстройки вместе с
    основанием, и прямой порядок дошёл бы до уже погашенных повторно, записав в
    журнал пустое «off то же off». Ответ при этом 200, и тест не заметит.
    """
