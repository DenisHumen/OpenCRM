...
единственного места, где ключ дописывался. `docs/osnovy/03-baza-dannyh.md` при этом
...
POLNYY_BEZ_KLYUCHA: dict[tuple[str, str], str] = {
    ...
    (
        "fayly.py",
        "Document.kind",
    ): "сгруппировано по виду бланка: строка — это группа, а вид уникален по построению GROUP BY",
    (
        "vozvraty.py",
