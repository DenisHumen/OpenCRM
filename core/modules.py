...
MODULES: tuple[Module, ...] = (
    ...
    # Хранилище вторых факторов. Ни от чего не зависит: ключ от банка не
    # относится ни к клиенту, ни к заявке. Выключен по умолчанию — как почта и
    # телефония: заводить хранилище чужих доступов должны решением, а не
    # обнаружить его включённым. Связь с напоминаниями мягкая, проверкой
    # `is_enabled` в службе: без напоминаний ключи работают, просто некому
    # сказать «пора сменить».
    Module(key="keys", default=False),
    # Файлы. Ни от чего не зависит и включён: раздел показывает то, что уже
    # лежит на диске, — работы досок, вложения карточек, снимки товаров. Даже
    # у фирмы без единого своего файла он отвечает на вопрос «чем занято
    # место», ради которого его и открывали до сих пор.
    Module(key="files", default=True),
)
