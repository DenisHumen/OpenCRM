...
MODULES: tuple[Module, ...] = (
    ...
    Module(key="tasks"),
    # Шаблоны сообщений: типовой ответ, ссылка на доску, напоминание об
    # оплате. Включены по умолчанию, в отличие от почты и телефонии: тем
    # нужна настроенная снаружи железка, а шаблону — только клавиатура, и
    # пустой раздел с кнопкой «новый шаблон» это уже работающая функция, а
    # не обещание. Стоят на клиентах: подстановки называют клиента и его
    # заявку, и без них шаблон подставлять нечего.
    ...
    # Накладные: бумага, по которой товар физически уезжает со склада или
    # приезжает на него.
    #
    # **Свой переключатель, хотя накладная — вид бланка.** Прецедент рядом:
    # `orders` тоже вид бланка и тоже со своим блоком. Довод один и тот же —
    # мастерская, чинящая телефоны, выдаёт квитанции и не отгружает ничего по
    # накладным; магазин отгружает по накладным и квитанций не печатает. Держать
    # их одним переключателем значит навязать каждому из двоих чужую половину
    # меню.
    #
    # Стоит на бланках жёстко: номер, статусы, печать, поиск сканом и публичная
    # страница берутся оттуда готовыми — без бланков накладной нет вовсе.
    #
    # Со складом связь МЯГКАЯ, как у заказов: накладная на услуги складу не
    # нужна, и объявлять зависимость жёсткой значило бы запретить накладные
    # тому, у кого склада нет. Выражается проверкой `is_enabled` в
    # `waybill_service`.
    ...
    # Наблюдение за сервером: оповещения в Telegram, графики и поиск по
    # логам. Ни от чего не зависит: он про машину, а не про заявки. По
    # умолчанию выключен — на VPS с двумя гигабайтами полный набор
...
PRESETS: tuple[Preset, ...] = (
    ...
    # Накладные — в оптовом и производственном, но НЕ в розничном.
    #
    # Разница не в размере, а в том, чем сопровождается отгрузка. В рознице
    # товар уходит через кассу, и бумагой служит чек: накладная там лишний
    # экран, который придётся объяснять каждому новому продавцу. В опте и на
    # производстве товар уезжает партией и с сопроводительным документом —
    # без него получатель не примет.
    Preset(
        key="wholesale",
        modules=(
