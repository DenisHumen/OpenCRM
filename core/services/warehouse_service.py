...
#: Потолок количества в тысячных долях: два миллиона единиц товара. Смысл тот
#: же, что у `MAX_MONEY` у денег, — это защита не от бизнеса, а от опечатки.
...
#:
#: Довод верный, а число было неверным, и пятисотка оставалась — просто на
#: другом пороге. Стояло `10**12` при колонке `stock_moves.quantity_milli` типа
#: `Integer` (INT, предел 2 147 483 647): всё, что между, проходило эту проверку
#: и упиралось в MySQL — 1264 «Out of range value». Про это прямо сказано в
#: модели рядом (`database/models/warehouse.py`): «отдельное движение больше
#: двух миллионов единиц туда не влезет».
#:
#: Два миллиарда тысячных — это 2 000 000 единиц в одном движении. Для штук,
#: килограммов и метров розницы запас громадный; понадобится больше — колонку
#: расширяют до BigInteger, и тогда меняется и это число, и оно же в
#: `RAZRESHENO_VYSHE_INT` в `tests/test_potolki.py`.
...
#: получит два вида артикула вместо одного. Разбор — в docs/bloki/19-sborka-zakaza.md §Р1.
SKU_PRISTAVKA = "A-"
SKU_ZNAKOV = 6
...
def write_off_materials(
    ...
    **Себестоимость снимается на строку бланка.** Это снимок момента, а не
    ...
    # Третье место с тем же устройством — после переезда и проведения заказа.
    # Правило общее и следует прямо из «остаток не хранится»: раз он считается
    # запросом, у любой проверки «хватает ли» два шага, и между ними окно.
    # Значит проверка остатка обязана НАЧИНАТЬСЯ с занятия товара.
    #
