...
#: Строка «что это за бумага» для снимка печати. Заголовок акта человек может
#: переписать при заведении — «Акт сдачи-приёмки», «Наряд-заказ»; отраслевого
#: словаря здесь не заводим по той же причине, по которой его нет у квитанции.
#:
#: Совпадает слово в слово с `ACT_PRINT_STRINGS["en"]["title"]` в
#: `web/api/routes/documents.py`, и это не совпадение: печатная форма берёт
#: заголовок из своего словаря по языку бумаги, а ЭТА строка ложится в запись
#: документа и показывается в списке CRM. Разойдись они — один и тот же акт
#: назывался бы в списке одним, а на бумаге другим.
