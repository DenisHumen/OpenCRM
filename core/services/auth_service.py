...
def disable(db: Session, actor: User, user_id: int) -> User:
    # Инвариант «раздавать права всегда есть кому» проверяется и здесь.
    #
    # Четыре двери из шести были закрыты — снять `roles.manage` с последней
    # роли нельзя, перевести её единственного носителя нельзя, — а отключить и
    # уволить его самого было можно. Итог тот же: система остаётся без
    # управляющего доступами, и восстановить её можно только с доступом к
    # серверу.
    #
    # Прежде здесь стояло, что закрыть это нельзя, иначе назначение первого
    # «гендиректора» станет необратимым. **Это неверно:** у управляющего есть
    # `roles.manage`, то есть он вправе назначить преемника сам. Выход из
    # положения — дать право второму, потом увольнять первого; отказ его и
    # называет.
    user = _get_manager(db, user_id)
    from core.services import permissions_service
...
def delete_user(db: Session, actor: User, user_id: int) -> None:
    ...
    # Инвариант «раздавать права всегда есть кому» — тот же, что в `disable`:
    # уволенный управляющий оставляет систему без управляющего так же, как
    # отключённый.
    from core.services import permissions_service
