...
def _sync_feed_note(db: Session, call: PhoneCall) -> None:
    ...
    Автор записи — оператор, если станция его назвала, и пусто, если нет. Это
    один из двух случаев во всей системе, где пустой автор законен
    (``SOURCE_TELEPHONY_WEBHOOK`` в ``database/models/audit.py``): событие шлёт
    АТС, человека за ним нет. Везде остальное пусто — это непротащенный
    исполнитель, и `audit_service.assert_actor` такое не пропускает.
    """
