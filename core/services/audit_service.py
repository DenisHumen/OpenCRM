...
#: Файл выпущен наружу по ссылке. В `entity_label` — какой файл и что по
#: ссылке можно: смотреть или ещё и скачать.
...
#: Возврат проведён: товар вернулся на склад, деньги — клиенту. Спрашивают
#: об этом чаще, чем о самой отгрузке: «почему на складе стало больше, чем вчера».
ACTION_RETURN_POSTED = "return.posted"
...
#: Бумага, заведённая по ошибке, удалена целиком — журнал объясняет дырку в нумерации.
ACTION_DOCUMENT_DELETED = "document.deleted"
#: Накладная проведена: товар физически покинул склад или приехал на него.
#:
#: Отдельно от `ACTION_ORDER_CLOSED`, хотя оба означают «склад двинулся».
#: Вопрос, который задают журналу, звучит не «когда двинулся склад», а «кто
#: отпустил товар по сто восемнадцатой» — и ответ на него обязан находиться по
#: виду бумаги, а не перебором строк одного общего действия.
ACTION_WAYBILL_POSTED = "waybill.posted"
#: Выписано сторно. Спрашивают об этом чаще, чем о самом проведении, — ровно как
#: у отмены заказа: «почему на складе стало больше, чем вчера».
ACTION_WAYBILL_REVERSED = "waybill.reversed"
...
#: смотрел код перед тем, как оттуда ушли деньги» задают именно так. Повторные
#: показы одного ключа одним человеком сходятся в одну запись за окно
#: (`klyuchi_service.POKAZ_OKNO_SEKUND`) — иначе один открытый экран, который
#: пересобирает код каждые тридцать секунд, дал бы сто двадцать записей в час.
...
# транзакции, что и само изменение, и строка длиннее колонки — это не кривая
# запись в журнале, а отказ **основной операции**: `Data too long for column`.
# То есть сотрудник не смог бы провести акт из-за того, что кто-то завёл
# длинное имя действия. Половина полей, обрезанная «на глазок», оставляет ровно
# ту же дыру — поэтому предел берётся у каждой колонки, какая сюда попадает.
