...
PRESETS: dict[str, dict] = {
    "universal": {
        "name": "Universal",
        "hint": "Fits almost any business — start here and rename to suit",
        "stages": [
            ("new", "New", KIND_OPEN),
            ("in_progress", "In progress", KIND_OPEN),
            ("ready", "Ready", KIND_OPEN),
            ("done", "Completed", KIND_WON),
            ("cancelled", "Declined", KIND_LOST),
        ],
    },
    "services": {
        "name": "Service and repair",
        "hint": "Repair shops, workshops, on-site work",
        "stages": [
            ("request", "Request", KIND_OPEN),
            ("diagnostics", "Diagnostics", KIND_OPEN),
            ("approval", "Quote approval", KIND_OPEN),
            ("repair", "In progress", KIND_OPEN),
            ("ready", "Ready for pickup", KIND_OPEN),
            ("issued", "Handed over", KIND_WON),
            ("refused", "Declined", KIND_LOST),
        ],
    },
    "beauty": {
        "name": "Appointments",
        "hint": "Salons, studios, practitioners — work booked by time slot",
        "stages": [
            ("request", "Enquiry", KIND_OPEN),
            ("booked", "Booked", KIND_OPEN),
            ("confirmed", "Confirmed", KIND_OPEN),
            ("came", "Arrived", KIND_OPEN),
            ("served", "Served", KIND_WON),
            ("no_show", "No-show", KIND_LOST),
        ],
    },
    "shop": {
        "name": "Shop",
        "hint": "Retail and online store: from order to handover",
        "stages": [
            ("new", "New order", KIND_OPEN),
            ("paid", "Paid", KIND_OPEN),
            ("packed", "Packed", KIND_OPEN),
            ("shipped", "Shipped", KIND_OPEN),
            ("received", "Received", KIND_WON),
            ("cancelled", "Cancelled", KIND_LOST),
        ],
    },
    "agency": {
        "name": "Studio and agency",
        "hint": "Project work with a quote and hand-off",
        "stages": [
            ("lead", "Lead", KIND_OPEN),
            ("quote", "Quote", KIND_OPEN),
            ("in_progress", "In progress", KIND_OPEN),
            ("review", "Review", KIND_OPEN),
            ("won", "Won", KIND_WON),
            ("lost", "Lost", KIND_LOST),
        ],
    },
...
def apply_preset(
    ...
    if dropped:
        ...
        # Двумя запросами, а не циклом по заявкам. Разбор цены и порядка — в
        # `deals_repo.pereselit_iz_etapov`; здесь про то, ЧТО именно
        # записывается и почему.
        #
        ...
        # заявки появилась бы строка «этап сменился», которой никто не делал.
        # Перестройка воронки — не работа по заявке, а настройка системы; её
        # место в журнале действий, а не в переписке с клиентом.
        deals_repo.pereselit_iz_etapov(db, dropped, target, actor.id if actor else None)
        db.flush()
