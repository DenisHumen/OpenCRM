...
def _write_state(db: Session, key: str, enabled: bool, user: User, *, was: bool) -> None:
    ...
    Соседей `affected_by` отбирает только с другим значением, а сам блок
    приходит любым: включение включённого писало в журнал «Включено →
    Включено» и затирало, кто переключал его на самом деле (проба 06.09.2026).
    """
    if was == enabled:
        return
