...
AREA_ACTIONS: dict[str, tuple[str, ...]] = {
    ...
    # Накладные. Набор тот же, что у заказов, и по тем же доводам: `issue` —
    # это провести, то есть двинуть склад, и оно отделено от набора позиций
    # (кладовщик собирает, отпускает старший); `view_amounts` — потому что
    # менеджер ведёт отгрузку, а закупочную цену видеть не обязан.
    #
    # `delete` в базовом наборе есть, но удалить накладную можно только пока она
    # черновик — после проведения бумага неизменяема, и удаление тоже
    # изменение. Проверка стоит в службе: право говорит, КОМУ можно, а не КОГДА.
    ...
    # Финансы. Три действия вместо четырёх базовых, и оба отличия намеренны.
    #
    # `edit` и `delete` не объявлены, потому что операцию не правят и не
    # удаляют: ошибку исправляют обратной операцией (разбор — в модели
    # `FinanceOperation`). Право, которое ничего не даёт, хуже его отсутствия —
    # тот же довод, что у журнала действий.
    #
    # `manage` — распоряжаться справочником статей и планами. Решение
    # структурное, вроде «завести юрлицо» или «завести склад»: расходы заносят
    # каждый день, а статьи заводят раз в год, и это разные полномочия.
    #
    # `view_amounts` здесь нет и быть не может. У заявок и склада суммы отделены
    # от раздела, потому что «ведёт заявку, но не видит маржу» — осмысленная
    # просьба. Финансы это деньги целиком: раздел без сумм — пустая таблица,
    # поэтому `view` и есть право на деньги (так же закрыт отчёт о выручке).
    "finance": (VIEW, CREATE, MANAGE),
    # Переписка с клиентами. Три действия, и каждое отсутствующее — намеренно.
    #
    # `edit` и `delete` не объявлены: отправленное сообщение не правят и не
    # удаляют. Клиент его уже прочитал, и «исправить» переписку задним числом
    # значило бы завести способ подделать доказательство. Ошиблись — пишут
    # следом, как в жизни.
    #
    # `view_others` НЕ объявлено, и это разрешение спора внутри самого замысла.
    # В раннем разборе оно значилось — по образцу заявок, где у карточки есть
    # ответственный. Но решение от 12 августа отменило и закрепление диалога, и
    # ответственного: конфликты показываются баннером, люди договариваются
    # сами. Значит у диалога нет владельца, а праву «видеть чужие» нечего
    # фильтровать. Право, которое ничего не даёт, хуже его отсутствия — тот же
    # довод, что у журнала действий и у финансов.
    #
    # `manage` — настройки бота: токен, чат сводки, подключение. Отдельно от
    # `create`: писать клиентам каждый день, а подключать бота раз и навсегда.
