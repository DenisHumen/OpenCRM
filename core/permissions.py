...
MANAGE = "manage"
#: Выпустить наружу по ссылке. Отдельно от `view`: смотреть файл внутри фирмы и
#: положить его в интернет по адресу, который перешлют кому угодно, — разные
#: полномочия, и второе отзывают чаще, чем первое.
SHARE = "share"
...
AREA_ACTIONS: dict[str, tuple[str, ...]] = {
    ...
    # `delete` не объявлен: бумага не удаляется, она ОТМЕНЯЕТСЯ
    # (`STATUS_CANCELLED`) или сторнируется. Ручки удаления нет, функции в
    # службе нет, права никто не спрашивает — см. ниже про пустые права.
    "documents": (VIEW, CREATE, EDIT, ISSUE),
    ...
    "orders": (VIEW, CREATE, EDIT, ISSUE, VIEW_AMOUNTS),
    ...
    # бумага не исчезает, это правило учёта (`docs/bloki/17-nakladnye.md` §7.1).
    # Убирать ли действие из матрицы — решение владельца: оно тянет за собой
    # пресеты и уже выданные роли. Развилка записана в `docs/dizayn/20-udobstvo-i-spravka.md`.
    ...
    # Почта — ЗЕРКАЛО сервера, и довод здесь тот же, что у телеграма строкой
    # выше. `edit` не объявлен: правка письма задним числом была бы способом
    # подделать переписку. `delete` — способом потерять её, оставив на сервере:
    # из ящика письмо никуда не денется, а из ленты клиента исчезнет.
    "mail": (VIEW, CREATE),
    # Журнал звонков только дописывается: звонок был, и убрать его из истории
    # значит убрать факт. Править запись можно — привязать к клиенту, поставить
    # исход, — а стирать нечего.
    "telephony": (VIEW, CREATE, EDIT),
    # Раздел только ПОКАЗЫВАЕТ состояние стека наблюдения. Тревоги приходят
    # снаружи, цели описаны конфигом на диске; заводить и править в интерфейсе
    # нечего, и правом это не станет.
    "monitoring": (VIEW,),
    # Ключи двухфакторки. Право на раздел — только вход в него: ЧТО именно
    # видно внутри, решает список доступа у каждого ключа, а не матрица
    # (`database/repositories/klyuchi.py`). Иначе `view` открывал бы все чужие
    # вторые факторы разом — ровно то, ради чего хранилище и заводят порознь.
    # `manage` — раздавать доступ и вести категории: пустить человека в чужой
    # ключ и завести свой это разные полномочия.
    "keys": BASE_ACTIONS + (RESTORE, MANAGE),
    # Файлы. `edit` не объявлен: файл не правят — его заменяют новым, а имя
    # ему даёт то, откуда он пришёл. `create` — завести папку и принести файл:
    # и то, и другое кладёт байты на диск, и делить их нечем. ЧТО именно видно
    # в дереве, решают права на разделы, из которых файлы пришли: без права на
    # клиентов ветки «Клиенты» нет вовсе.
    # `share` — выпустить наружу по ссылке; она уходит из фирмы, и отзывают её
    # чаще, чем закрывают раздел.
    "files": (VIEW, CREATE, DELETE, SHARE),
}
...
SYSTEM_AREAS: tuple[Area, ...] = (
    ...
    # (docs/ekspluatatsiya/15-kopii-s-shifrovaniem.md §6). Снятие копии остаётся под настройками.
