...
AREA_ACTIONS: dict[str, tuple[str, ...]] = {
    ...
    # Накладные. Набор и доводы те же, что у заказов.
    #
    # `delete` у бумаг — БУМАЖНОЕ право без работы за ним, и это записано здесь
    # честно, а не объяснено задним числом. Ни бланк, ни заказ, ни накладную
    # удалить нельзя ничем: ручки нет, функции в службе нет, права этого никто
    # не спрашивает — удаляются только строки перечня, и под `edit`. Выданная
    # бумага не исчезает, это правило учёта (`docs/17-nakladnye.md` §7.1).
    # Убирать ли действие из матрицы — решение владельца: оно тянет за собой
    # пресеты и уже выданные роли. Развилка записана в `docs/20-udobstvo-i-spravka.md`.
