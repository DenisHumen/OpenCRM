...
class UpdateConfig:
    ...
    #: Сколько мегабайт должно быть свободно, чтобы вообще начинать.
    #:
    #: Обновление пишет на диск трижды: копия базы (на боевом объёме это
    #: гигабайт с лишним), слои нового образа и образ ворот. Старый образ при
    #: этом лежит до `prune`, то есть на пике их два. Кончившееся место — не
    #: гипотеза: образ панели мониторинга разом вырос с 744 МБ до 1.16 ГБ.
    #:
    #: Отказ по месту дёшев, если случился ДО первой записи, и очень дорог
    #: посреди дампа: оборванный дамп — обычный текстовый файл, и негодность у
    #: него не видна ничем, кроме отсутствующего хвоста.
    min_free_mb: int
    run_checks: bool
    require_ci: bool
    allow_dirty: bool
    ...
    def from_env(cls, environ: dict[str, str] | None = None) -> UpdateConfig:
        ...
        return cls(
            ...
            # обновление начнётся и упрётся в место где-то посередине.
            min_free_mb=int(get("MIN_FREE_MB", "2048")),
