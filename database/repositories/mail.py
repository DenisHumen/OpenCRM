...
def save_last_error(db: Session, account: MailAccount, text: str, at: datetime) -> None:
    ...
    **Почему не отдельная короткая сессия, как для фоновой задачи.** Второе
    соединение не видит того, что транзакция запроса ещё не зафиксировала, зато
    упирается в её замки: к моменту отказа транспорта запрос обычно уже пишет —
    любой запрос обновляет сотруднику «последний раз в сети» (`auth_service`),
    и эта правка висит незафиксированной. Поймано делом ещё на файловой базе:
    отдельная сессия ждала 5,5 секунды и отвечала `database is locked`, ошибка
    не сохранялась вовсе — то есть починка не работала именно в том случае,
