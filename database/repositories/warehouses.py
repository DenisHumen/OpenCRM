...
def zapert_zhivye(db: Session) -> None:
    """Занять все открытые склады до конца транзакции.

    **Спор идёт не о строке, а о том, сколько их останется.** Отказ «последний
    склад закрыть нельзя» держится счётом живых, то есть запросом: «ровно один»
    частичным индексом в MySQL не выразить. Между счётом и записью есть окно, и
    в него попадают двое — оба видят «складов два», оба проходят проверку, оба
    закрывают, и складов не остаётся вовсе.

    Замерено дуэлью: пять раз из пяти складов оставалось НОЛЬ. Тогда система не
    принимает ни одного прихода (`default_warehouse` отвечает `no_warehouse`), а
    заявка со строками перестаёт закрываться вовсе — списанию некуда идти.

    Замок на ВСЕ живые строки, а не на закрываемую: второй дуэлянт ждёт первого,
    а дождавшись, видит уже один склад и получает честный отказ. Тот же приём и
    тот же разбор, что у последнего владельца (`repositories/users.py`).
    """
    db.execute(
        select(Warehouse.id).where(Warehouse.deleted_at.is_(None)).with_for_update()
    ).all()
