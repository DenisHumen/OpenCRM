...
def _zapert_vladeltsev(db: Session) -> None:
    ...
    **Без этого защита не работает на MySQL, и это проверено гонкой.** Условие
    ...
    На SQLite беды не было по случайности устройства: писатель там один, второй
    ждёт и пересчитывает уже после чужой записи. То есть защиту держала не
    проверка, а однопоточность движка, — и переезд на MySQL её снял молча.
    ...
    if db.bind.dialect.name == "sqlite":
        # У файловой базы `FOR UPDATE` нет вовсе, а сериализация уже есть.
        return
