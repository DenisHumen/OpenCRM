...
def find_candidates(
    ...
    **Порядок задан, и он не украшение.** Сначала совпавшие точной приметой
    ...
    if not conditions:
        return []
    # Нет точных примет — делить выдачу не на что, и ступени просто НЕТ.
    # Константа в `ORDER BY` уезжает в запрос параметром, а MySQL отвергает
    # такой запрос целиком: заказ по одному имени отвечал пятисоткой.
    stupeni = []
    if exact_conditions:
