...
#: ключа живёт здесь, вместе с остальным справочником: показывают источник
#: карточка клиента и отчёт, и ключ, известный одному лишь приёму, стоял бы в
#: них сырым словом `site`. Ровно так оно и было, пока приёмом заявок нельзя
#: было управлять из интерфейса.
SOURCE_SITE = "site"
