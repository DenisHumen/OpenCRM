...
# Тип этапа — то немногое, что НЕ настраивается. Названия у всех свои
# («диагностика», «клиент пришёл», «отправлен»), а отчёт по названию считать не
# может: воронка — все `open` по порядку, конверсия — доля `won`, потери — `lost`.
KIND_OPEN = "open"
KIND_WON = "won"
KIND_LOST = "lost"
