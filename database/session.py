...
def _make_engine():
    ...
    if url.startswith("mysql"):
        ...
        #
        # Поймано делом: на SQLite проверка двойного нажатия «Запросить доступ»
        # проходила, на MySQL — падала с `Duplicate entry ... for key
        # users.ix_users_email`. То есть на боевом сервере это уже работало
        # именно так, а набор тестов этого не видел, потому что гонялся на файле.
