...
def downgrade() -> None:
    # Индексы перед `drop_table` не снимаются, и это правило на все миграции
    # проекта. Причин две, и вторая решающая:
    #
    # 1. таблица уносит свои индексы сама — снимать их отдельно нечего;
    # 2. **MySQL не даёт снять индекс, по которому проверяется внешний ключ** —
    #    отказ 1553 «needed in a foreign key constraint». А индекс под внешний
    #    ключ у нас на каждом: `ix_share_views_share_link_id`,
    #    `ix_works_board_id`, `ix_clients_manager_id` — все они разом.
    #
    # Автогенератор alembic пишет снятие индексов, потому что писал он это под
    # SQLite, где порядок безразличен. На MySQL откат ломался ровно здесь: в
    # первой же строке, `DROP INDEX ix_share_views_share_link_id`. Порядок
    # таблиц при этом важен по-прежнему — от зависимых к тем, на кого ссылаются.
