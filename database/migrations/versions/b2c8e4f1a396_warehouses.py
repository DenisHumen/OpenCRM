...
def upgrade() -> None:
    ...
    #    движения. Название по-английски: интерфейс по умолчанию английский, а
    #    переименовать склад можно в первый же день.
    #
    #    Здесь стояло «Основной» с пояснением «язык интерфейса по умолчанию
    #    русский». Пояснение перестало быть правдой, а строка осталась — и
    #    именно она доезжает до человека: на свежей установке в докере точка
    #    входа гонит `alembic upgrade head` по пустой базе, все миграции
    #    отрабатывают, и `warehouse_service.seed_defaults` находит склад уже
    #    созданным. То есть засев в коде на боевом пути не участвует вовсе.
    #
    #    Правка литерала переписывает не историю схемы, а только то, что
    #    достанется БУДУЩИМ установкам. Уже поднятые базы не трогает никак:
    #    миграция у них отмечена и второй раз не пойдёт, а «Основной» там давно
    #    не умолчание, а название, за которым стоят чужие движения.
    warehouses = sa.table(
        "warehouses",
    ...
    op.bulk_insert(warehouses, [{"id": 1, "name": "Main", "is_default": True}])
