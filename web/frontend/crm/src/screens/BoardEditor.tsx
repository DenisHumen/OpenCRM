            ...
            {board.works.map((work: any) => (
              ...
              >
                <div className="work-media">
                  ...
                  {/* Обрезка доступна ЛЮБОЙ картинке, а не только «длинной».
                      Прежнее условие опиралось на порог «высота ≥ ширины во
                      столько-то раз» и утверждало, что короткая помещается в
                      своё место целиком. Это неправда: форму места задаёт
                      композиция, а мест в ней несколько и они разной формы —
                      одна и та же работа в одном месте видна целиком, а в
                      соседнем обрезается. Отсюда и жалоба: у одной работы
                      срезано название, у другой — кончики предметов, и подвинуть
                      окно было нечем.

                      Лишней кнопка не станет: окно обрезки берёт форму места, и
                      если работа помещается, ехать ему некуда — модалка это уже
                      умеет (`travel <= 0`). */}
                  {work.status === "ready" && work.kind === "image" && (
