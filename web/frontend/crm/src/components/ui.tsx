...
/** Кнопка удаления: круглая корзинка, по наведению разъезжается в красную
 *  пилюлю со словом. Перевод `uiverse.io/vinodjangid07/smart-emu-83` (владелец,
 *  07.09.2026); порядок перевода — docs/dizayn/18-chuzhie-komponenty.md.
 *
...
export function KnopkaKorziny({
  ...
  onClick: () => void;
  /** Что именно удаляют — для читалки и всплывающей подписи. */
  title?: string;
  /** Слово на разъехавшейся пилюле. По умолчанию «Удалить»; своё нужно там, где
   *  действие называется иначе, — «Закрыть статью» с надписью «Удалить» на
   *  кнопке была бы неправдой. Коротко: пилюля шириной в одно слово. */
  podpis?: string;
  disabled?: boolean;
}) {
  const { t } = useApp();
