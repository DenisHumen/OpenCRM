 ...
 *
 * Устройство — docs/ustroystvo/12-zhivye-obnovleniya.md §2, §6, §11. Коротко:
 *
 ...
 *
 * **Число живёт здесь, и только здесь.** У сводки стояла своя такая же пара
 * «таймер + слушатель видимости» с тем же периодом — не подстраховка, а
 * удвоение: при выключенной живости самая дорогая ручка дёргалась по два раза
 * подряд.
 *
 * Две минуты, а не десять секунд. **Замерено: сводка стоит 23 запроса к базе**
 * против четырёх у списка бумаг — один заход собирает деньги, воронку целиком,
 * задачи, клиентов и просмотры за неделю. Минута дала бы 1380 запросов в час с
 * одной вкладки. Столько же, сколько у проверки свободного места
 * (`lib/app.tsx`), и это не совпадение: разные интервалы у двух фоновых
 * перезапросов означали бы два числа, которые кто-то однажды начнёт сближать.
 */
 ...
 * загруженного. Хук наблюдает, полями не управляет (docs/ustroystvo/12-zhivye-obnovleniya.md §8).
 *
