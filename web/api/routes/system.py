...
def purge_storage(_: User = Depends(require_root), db: Session = Depends(get_db)):
...
@router.get("/files")
def list_media_files(_: User = Depends(require_root), db: Session = Depends(get_db)):
...
def delete_media_file(work_id: int, _: User = Depends(require_root), db: Session = Depends(get_db)):
