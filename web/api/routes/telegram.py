...
def istochniki(
    ...
    """Какие метки источника встречаются и по сколько диалогов на каждой.

    Отбор по метке ручка диалогов принимала и раньше, но экрану неоткуда было
    узнать, какие метки существуют: их придумывает владелец, раздавая ссылки
    и наклейки, и через полгода сам не помнит полного списка. Отбор без такого
    списка можно предложить только полем ввода — то есть предложить угадывать.

    Счёт приходит вместе с метками, потому что он и есть ответ на вопрос, ради
    которого метки заводят: сколько пришло с наклейки, а сколько с сайта.
    """
    return {
        "items": [
            {"source": metka, "count": skolko}
            for metka, skolko in telegram_repo.istochniki(db)
        ]
    }
