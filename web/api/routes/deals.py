...
def _card(db: Session, deal, user: User) -> dict:
    ...
    # Есть ли строки — карточке, а не только блоку склада. Поле «Сумма» она
    # запирала по ответу списка строк, а тот при выключенном складе молчит:
    # поле открывалось, сервер отвечал `amount_from_lines`, и починить сумму
    # человек не мог вовсе. В списках и на канбане счёт не кладём — там это
    # запрос на каждую строку.
    data["has_lines"] = deal_lines_service.est_stroki(db, deal.id)
