...
from web.api.deps import get_current_user, get_db, require_perm

# Читать настройки — одно право, менять — другое. Раздел целиком закрыт на
# `settings.view`, а каждая правка дополнительно требует `settings.manage`:
# бухгалтеру нужны реквизиты и валюта, а логотип сайта и режим обслуживания —
# нет, и разделение позволяет дать первое, не отдавая второго.
router = APIRouter(
    prefix="/settings",
    tags=["settings"],
    dependencies=[Depends(require_perm("settings", "view"))],
)

#: Правка настроек. Отдельной зависимостью, чтобы не повторять строку девять раз.
...
@router.post("/maintenance", dependencies=[manage])
def set_maintenance(
    payload: MaintenanceIn,
    # get_current_user, а не ещё одна проверка права: право уже спросили
    # зависимости маршрута, здесь нужно только имя — кто закрыл сайт.
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """Закрыть сайт на работы или открыть обратно. Требует `settings.manage`."""
    return maintenance_mode.set_mode(db, payload.enabled, payload.note, user.name or user.email)


@router.patch("", dependencies=[manage])
...
@router.delete("/logo", dependencies=[manage])
...
@router.post("/site-logo/fetch", status_code=201, dependencies=[manage])
...
@router.delete("/site-logo", dependencies=[manage])
...
@router.delete("/og-image", dependencies=[manage])
