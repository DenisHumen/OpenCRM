...
def _collect_ratelimiter(out: Metrics) -> None:
    ...
    # То же самое, но СОСТОЯНИЕМ, а не историей — и вот зачем.
    #
    # Счётчик выше живёт в памяти ПРОЦЕССА, а Prometheus скребёт один адрес:
    # nginx отдаёт запрос случайному рабочему процессу. При нескольких воркерах
    # ряд счётчика заскачет между их независимыми значениями (5, 0, 3, 0…), а
    # для типа `counter` каждое понижение читается как перезапуск — `increase`
    # начнёт показывать прирост там, где его нет, и тревога зазвонит на ровном
    # месте. Не «когда-нибудь»: при четырёх воркерах это происходит с первого же
    # среза после первой в жизни неудачи Redis.
    #
    # Состояние такой беды не имеет по построению: «сбоил ли ограничитель в
    # последнюю минуту» — вопрос о ТЕКУЩЕЙ обстановке, и ответ на него у всех
    # процессов одинаков, потому что Redis у них общий. Какой из них ответит на
    # опрос, значения не имеет.
    #
    # Счётчик при этом оставлен: на одном процессе он отвечает на другой вопрос
    # («сколько раз за жизнь»), полезен в разборе и никому не мешает. Тревога
    # переехала на состояние — см. `RateLimiterUnavailable` в правилах.
    out.add(
        "opencrm_ratelimiter_recently_unavailable",
        1 if ratelimit.recently_unavailable() else 0,
        help_text=(
            "1 — ограничитель не достучался до Redis в последнюю минуту. "
            "Состояние, а не история: устойчиво к нескольким рабочим процессам."
        ),
    )
...
def _collect_bezopasnost(out: Metrics) -> None:
    ...
    **Один счётчик с меткой `vid`, а не семь отдельных имён.** Метка позволяет
    сложить всё разом (`sum(rate(...))` — «идёт ли на нас что-нибудь») и тут же
    разложить по видам (`by (vid)` — «что именно идёт»). Семь имён дали бы
    только второе, а первый вопрос задаётся ночью и первым.

    Значения берутся из Redis, а не из памяти процесса, — довод целиком в шапке
    `core/bezopasnost.py`: при нескольких рабочих процессах счётчик в памяти
    даёт пилу, которую `increase()` читает как перезапуски.

    Ряды выводятся ВСЕГДА, включая нулевые. Отсутствующий ряд на графике
    выглядит как «данных нет», а ноль — как «ничего не происходит», и это
    разные новости: первая означает, что сломался сбор, вторая — что всё тихо.

    **Человеческое описание вида в метку НЕ кладётся**, хотя соблазн есть:
    легенда Grafana показала бы «неверная пара почта/пароль» вместо
    `vhod_promah`. Цена этому — потеря истории. Метка входит в тождество ряда,
    и первая же правка формулировки (запятая, буква «ё») завела бы НОВЫЙ ряд, а
    старый оборвался бы на полуслове. График подбора обязан быть непрерывным
    годами; описания живут в `bezopasnost.VIDY` и в подписях панелей, где их
    можно менять сколько угодно.
    """
    from core import bezopasnost

    schetchiki = bezopasnost.snyat()
    for vid in bezopasnost.VIDY:
        out.add(
            "opencrm_security_events_total",
            schetchiki.get(vid, 0),
            help_text=(
                "События безопасности с начала жизни установки: промахи по "
                "паролю, PIN и номеру бланка, а также отказы ограничителя."
            ),
            kind="counter",
            labels={"vid": vid},
        )
...
def _collect_live(out: Metrics) -> None:
    ...
    теряются (`docs/ustroystvo/12-zhivye-obnovleniya.md` §11), и без счётчика это состояние видно
