...
def _edinica_raboty():
    ...
    На SQLite `commit` — запись в местный файл, она обычно успевает раньше, чем
    браузер пришлёт следующий запрос. На MySQL это сетевой круг, и следующий
    запрос его обгоняет. Замерено (вход, сразу `GET /auth/me`, двадцать раз):
    SQLite — 1 промах из 20, MySQL — 9 из 20, с паузой в секунду — 0 из 20.
