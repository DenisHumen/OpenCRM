...
def _edinica_raboty():
    ...
    try:
        yield db
        # «Последний раз в сети» — последним делом перед фиксацией, и место
        # выбрано, а не случилось. Отметка ставится на КАЖДОМ обращении
        # сотрудника, и пока она писалась через ORM, запись уходила в базу на
        # первом же `autoflush` — в начале запроса — и запирала строку
        # сотрудника до конца транзакции. Соседний запрос ТОГО ЖЕ человека
        # (браузер шлёт их пачкой) упирался в эту строку, ждал полсотни секунд
        # и получал `(1205, Lock wait timeout exceeded)` и пятисотую. Поймано
