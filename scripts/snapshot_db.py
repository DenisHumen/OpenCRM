...
откатывает и код, и базу (`deploy/updater.py`). На SQLite копию снимает
`sqlite3 .backup` прямо в `docker/entrypoint.sh`; на MySQL до этой правки не
снималось НИЧЕГО — снимок стоял внутри проверки «файл базы существует», то есть
только для файловой базы. На стенде, работающем на MySQL, это видно в журнале
запуска: старая копия SQLite-файла и следом миграции по MySQL.
...
alembic, а прочитать `alembic_version` в MySQL из точки входа нечем: `sqlite3`
для этого не годится по очевидной причине. То есть Python на этом шаге
появляется в любом случае, и вопрос лишь в том, звать ли из него ещё и внешний
клиент.
...
#: `mail_messages`. Столько же берёт пачка в `scripts/migrate_to_mysql.py`, и это
#: не совпадение: ограничение одно и то же.
RAZMER_PACHKI = 500
