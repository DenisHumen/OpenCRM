...
#: Столбцы, которые приложение пишет САМО СЕБЕ и которые обязаны меняться между
#: снятием копии и сверкой. Сравнивать их — не строгость, а ошибка в постановке
#: вопроса.
#:
#: **Найдено на боевом переезде.** Сверка встала на этом:
#:
...
#: `auth_service` пишет `last_seen_at` на ЛЮБОМ запросе вошедшего (не чаще раза
#: в минуту), а root проходит в закрытый сайт по устройству режима
#: обслуживания. То есть достаточно открытой вкладки CRM у того, кто переезд и
#: запускает, — и сверка не сойдётся НИКОГДА.
#:
#: Признак, по которому столбец сюда попадает, один: его пишет само приложение
#: как отметку о себе, у величины НЕТ ИСТОРИИ, и следующее событие затирает её
#: целиком. Потерять такую отметку стоит ровно одного лишнего опроса.
#:
#: Чего здесь быть НЕ МОЖЕТ: денег (`*_minor`), количеств (`*_milli`), ставок
#: (`rate_bp`), ключей и вообще всего, что вводит человек. За этим следит
#: `tests/test_migrate_to_mysql.py`, а не аккуратность того, кто правит список.
#:
#: Расхождение по ним не проглатывается молча — оно пересчитывается и
#: называется отдельной строкой в итоге сверки.
HOZYAYSTVENNYE = {
    # присутствие: «в сети» и «последний раз заходил»
    ("users", "last_seen_at"),
    ("user_sessions", "last_seen_at"),
    # отметка «когда последний раз опрашивали ящик». Сами письма при этом
    # сверяются как обычно: пропавшее письмо — это пропавшая СТРОКА, и его
    # поймает сличение по ключу.
    ("mail_accounts", "last_sync_at"),
}
...
def sverit_postrochno(source, target, table) -> list[str]:
    ...
    **Отсюда же и польза для второй беды.** Строка, заведённая в CRM уже после
    ...
    vsego = 0
    #: Сколько раз разошлись хозяйственные отметки. Не расхождение, но и не
    #: тишина: число называется в итоге сверки.
    hozyaystvennyh = 0
    ...
    while a is not None or b is not None:
        if b is None or (a is not None and odin_kluch and a[0] < b[0]):
            ...
            if a != b:
                ...
                # оно бывает и на честном переносе (см. `DOPUSK_FLOAT`), а
                # хозяйственные отметки обязаны разойтись у всякого, кто ведёт
                # переезд с открытой вкладкой CRM (см. `HOZYAYSTVENNYE`).
                otlichiya = []
                for stolbec, imya, bylo, stalo in zip(stolbcy, imena, a, b):
                    if _ravno(stolbec, bylo, stalo):
                        continue
                    if (table.name, imya) in HOZYAYSTVENNYE:
                        hozyaystvennyh += 1
                        continue
                    otlichiya.append(f"{imya}: было {bylo!r}, стало {stalo!r}")
                if otlichiya:
                    skazat(
    ...
    # Говорим вслух, а не проглатываем: человек должен видеть, что сверка что-то
    # пропустила намеренно, и знать — что именно.
    if hozyaystvennyh:
        print(
            f"  {table.name}: разошлись хозяйственные отметки ({hozyaystvennyh}) — "
            "присутствие и время последнего опроса. Это не данные: величина без "
            "истории, её затирает следующее событие."
        )
