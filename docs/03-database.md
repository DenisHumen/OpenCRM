...
Копия снимается **по тому адресу, по которому работаем**, а не по наличию файла:

| База | Чем снимается | Куда |
|---|---|---|
| SQLite | `sqlite3 .backup` | `opencrm.db.pre-migrate-<ревизия>` |
| MySQL | `python -m scripts.snapshot_db dump` | `mysql.pre-migrate-<ревизия>.sql` |

Прежде здесь стояло «если файл базы существует», то есть копия снималась только
для SQLite. На установке, переехавшей на MySQL, это давало худший из исходов:
файл SQLite после переезда остаётся лежать на месте (его нарочно не трогают),
условие срабатывало — и снималась копия базы, которой никто не пользуется, а
следом шли миграции по MySQL. Копии рабочей базы не было вовсе, и неудачное
обновление откатывало код, а базу откатывать было нечем.

Правила для обеих одинаковы: **имя по ревизии, с которой уходим; существующую не
...
честный: ничего не требует, лежит одним файлом, на нагрузке одной студии не
уступает. Но он отвечал не на тот вопрос. У SQLite **ровно один писатель**,
поэтому на ней невозможен второй рабочий процесс, а значит установка, начатая
«попроще», рано или поздно упирается в переезд — то есть в закрытый сайт и самую
опасную операцию, какая в проекте есть. Дешевле поставить сразу то, на чём
система будет работать.

Цена названа вслух: образ MySQL — около 600 МБ, сервер — порядка 400 МБ памяти
сверх сайта. На машине, где памяти меньше двух гигабайт, SQLite остаётся
законным выбором — она просто перестала быть умолчанием. Redis обязателен в
обоих случаях: в нём общий на все процессы счётчик попыток входа и PIN.

Ниже — про то, где разница между движками видна на деле.
...
Одной командой: `./opencrm.sh migrate-mysql` (или пункт 17 в меню; мастер установки
предлагает то же самое, если рядом уже лежит населённый файл).

### Требование, из которого выведено всё остальное

**Переезд не удался — боевой сервер продолжает работать на местной базе.** Не «вернём
руками», а продолжает — сам собой, потому что с неё и не уходил. Отсюда порядок, где
каждый шаг обратим, и одно решение, которое делает обратимость бесплатной:
`OPENCRM_DB_URL` переписывается **последним** действием, а не первым.

| Шаг | Что делает | Что при неудаче |
|---|---|---|
| 1 | поднимает MySQL и Redis **рядом** с работающим сайтом; службу `app` не трогает | ничего: сайт на файле |
| 2 | снимает копию SQLite и **проверяет её чтением** — `PRAGMA integrity_check` плюс настоящий запрос к данным | ничего: сайт на файле |
| 3 | строит схему в MySQL миграциями, отдельным процессом (не боевым приложением) | ничего: сайт на файле |
| 4 | переносит данные **из копии** | цель наполовину заполнена, сказано вслух; сайт на файле |
| 5 | сверяет: схема с моделями, число строк по каждой таблице, суммы по всем целым столбцам | ничего: сайт на файле |
| 6 | переписывает `OPENCRM_DB_URL` и пересоздаёт контейнер приложения | **автоматический возврат на SQLite** |
| 7 | ждёт `/healthz` и `"schema":"ok"` | **автоматический возврат на SQLite** |

На шагах 1-5 откатывать нечего, и это не фигура речи: приложение всё это время работает с
файлом, а «откат» состоит в том, чтобы ничего не делать. Возврат — единственное действие
во всём сценарии, и делает его скрипт (`rollback_to_sqlite`), а не человек по инструкции.

Как это выглядело раньше: `OPENCRM_DB_URL` переписывался при **выборе** базы, за десяток
шагов до самого переноса. Всё это время сайт работал на пустой MySQL — схему в ней
построили миграции, умолчания посеял старт, `/healthz` зелёный, `schema_check` доволен, а
человек видел пустую CRM. То есть переключение происходило до того, как переезд признан
удачным, и любая осечка требовала возврата руками.

### Почему перенос идёт из копии, а сверка — по живому файлу

Копия неподвижна, и переносить надо из неё: живой файл при этом не открывается на запись
ни разу — копию снимает `sqlite3 .backup` (читатель), а скрипт переноса открывает источник
с `mode=ro&uri=true`. Испортить оригинал переезд не может ни при каком исходе.

**А вот сверять надо ЖИВОЙ файл, и раньше это делалось неверно.** Сверка сравнивала ту же
копию с целью — то есть отвечала на вопрос «доехало ли то, что мы взяли», а выдавалась за
ответ на вопрос «всё ли, что есть у людей, доехало». Разница стоила данных: во время
переноса через экран CRM завели клиента, переезд объявил «число строк и суммы совпали по
каждой таблице» и «переезд завершён», а после переключения этого клиента в новой базе не
оказалось. Предупреждение на экране было — и проиграло зелёному итогу, потому что верят
итогу.

Ложной тревоги, ради которой сверку когда-то увели на копию, больше нет: **на время
переноса сайт закрывается на обслуживание** (`migrate_maintenance` в `opencrm.sh`, тот же
режим, который root включает руками). Писать в это время некому, живой файл неподвижен, и
«сошлось» перестаёт быть обещанием.

Оговорка названа вслух: root в закрытый сайт проходит по устройству режима, и запись,
сделанная им самим во время переезда, по-прежнему не доедет — но теперь её **назовёт
сверка**, а не потеряет молча.

### Что именно сверяется

`scripts/migrate_to_mysql.py --verify` — отдельный заход, а не «заодно с переносом»:
проверка, выполняющаяся только вместе с удачным действием, проверяет действие, а не его
итог.

0. **Осмотр источника: нет ли строк, ссылающихся в пустоту.** Задаётся ПЕРВЫМ и одному
   лишь файлу — ни MySQL, ни копии, ни закрытого сайта для вопроса не нужно, поэтому
   отказ на нём не стоит никому ничего. Отдельным заходом:
   `python scripts/migrate_to_mysql.py --source … --preflight`.

   Нужен потому, что SQLite следит за внешними ключами **только по просьбе**: приложение
   просит (`PRAGMA foreign_keys=ON` в `database/session.py`), а миграции не просят и не
   могут — alembic в batch-режиме пересоздаёт таблицы, и включённые ключи ломают этот
   приём. Окно узкое, но открытое.

   MySQL таким снисхождением не страдает. Проверено живым прогоном на настоящем докере:
   перенос доходил до `deals`, получал `1452 Cannot add or update a child row` и падал
   **посередине** — сайт к этому времени уже закрыт на обслуживание, цель наполовину
   заполнена, а в логе имя ограничения вместо имени беды. Исход не страшный (источник
   открыт только на чтение, приложение с него не уходило), но дорогой.

   Список ключей берётся из моделей, а не из схемы файла: у старой базы ограничение могло
   быть не создано вовсе, и спрашивать её же о том, что проверять, значило бы поверить ей
   на слово. Называются и строка, и значение, в которое она показывает:
   `deals.client_id -> clients.id: 60 строк ссылаются на несуществующие (например: id=27 -> 2212)`.
1. **Схема цели с моделями** (`database/schema_check`). Совпадение ревизий alembic этого
   не заменяет: ревизия говорит, какие шаги отмечены выполненными, схема — что из них
   получилось. Расходятся они ровно в нужном случае: миграция прошла наполовину, успела
   отметиться и оставила таблицу без колонки.
2. **Число строк по каждой таблице.**
3. **Суммы по каждому целочисленному столбцу.** Целые в этом проекте — это деньги
   (`*_minor`), количества (`*_milli`) и ставки (`rate_bp`), то есть ровно то, расхождение
   чего нельзя заметить глазами.
4. **Строка в строку.**

Четвёртый пункт добавлен потому, что первых трёх недостаточно, и это проверено делом:
одной заявке `+1`, другой `−1` — число строк то же, сумма столбца та же, сверка зелёная,
сайт переключён на базу с двумя неверными суммами. Агрегат слеп к любой порче, которая
сама себя компенсирует, и к любой перестановке значений между строками.

Построчный проход идёт **встречным ходом по первичному ключу**: обе стороны читаются
потоком в одном порядке, указатели двигаются навстречу. Память от размера таблицы не
зависит вовсе (2,7 млн строк проходят в том же объёме, что и тысяча), а расхождение
называется **поимённо** — `products: id=1 — price_minor: было 298334, стало 298335` — в
тот момент, когда встретилось. Строка, появившаяся в живом файле после снятия копии,
выглядит здесь как `clients: id=987654 есть в источнике, в цели НЕТ`: по числу «строк
стало на одну меньше» человек не поймёт, кого искать.

**Из сравнения не выпадает ни один столбец, и два вида сравниваются по-особому.**

`Float` компилируется в MySQL в `FLOAT` (четыре байта против восьми у SQLite), и
0,3333333333 возвращается как 0,33333334 **на честном переносе тоже**. Прежде такие
столбцы выбрасывались из сличения целиком — объявленная в коде, но всё же дыра:
проверено делом, `works.duration_sec` 12.5 в источнике против 99999.5 в цели давало
ЗЕЛЁНУЮ сверку. Теперь они сравниваются с относительным допуском `1e-6` (на порядок
грубее точности четырёхбайтного числа и на много порядков тоньше любой порчи, которую
стоит ловить): 0,33333334 проходит, 0,99 против 0,25 — нет.

`Boolean` читается **сырым целым** (`type_coerce` к `Integer`, то есть без
преобразователя типа; сам SQL не меняется). Область истинности из двух значений, а
хранится она числом: `products.is_service` = 1 в источнике против 2 в цели — обе стороны
SQLAlchemy превращает в `True`, а сумм по логическим столбцам не берут вовсе. Значение
вне области истинности проезжало переезд насквозь, и это не было объявлено нигде.

Остальное сходится побайтно, и это свойство схемы, а не удача, — см.
`database/types.py` (`DATETIME(6)`, `ExactString`, `LongText`).

Отсюда же формулировки на экране. Перенос говорит «итоги сошлись» и сразу добавляет, что
это ещё не годность базы; годной её объявляет только `--verify`, и его зелёная строка
называет источник, с которым сравнивала.

### Мелочи, на которых это держится

1. Все типы переносимые; Alembic-миграции написаны без диалектных веток.
2. Выборка идёт с явным `ORDER BY` по первичному ключу. Без него порядок строк внутри
   таблицы не определён ничем, а у `finance_operations` есть ссылка на саму себя
   (`corrects_id`): вставка держалась на совпадении «SQLite отдаёт строки в порядке rowid,
   а поправка всегда старше поправляемой».
3. Целевые таблицы очищаются перед вставкой: миграции не только строят схему, но и сеют
   (этапы воронки, роль root и её права, склад «Основной»), и вставка поверх упала бы на
   уникальности — причём не на первой таблице, а на третьей-четвёртой.
4. Файлы в `storage/` не затрагиваются — пути в БД относительные.
5. После переезда: `utf8mb4`, движок InnoDB — задаётся в Alembic `env.py` через
   `mysql_*`-аргументы, SQLite их игнорирует.
6. Незавершённый переезд виден: непустая `OPENCRM_MIGRATE_TARGET_URL` при
   `OPENCRM_DB_URL` на sqlite — это отметка «начато и не доведено», её показывает
   `./opencrm.sh doctor` строкой «переезд».

### Вторая половина переезда — общий счётчик попыток

Переезд на MySQL снимает запрет на несколько рабочих процессов, а вместе с ним снял бы
единственное, на чём держалась защита от подбора: счётчик попыток жил в памяти процесса.
Поэтому Redis в этом продукте обязателен и поднимается всегда — подробности в
[08-deployment.md](08-deployment.md) и в шапке `core/redis_client.py`.

## Снятие SQLite после переезда

Переезд боевого сервера — **разовая операция**, и код, который её делает, живёт
ровно до неё. Дальше остаётся один стек: MySQL и Redis. Этот раздел существует
затем, чтобы снятие было механическим списком, а не разбором того, что тут
одноразовое, а что нет.

**Условие, до выполнения которого не снимать ничего.** Пока боевой работает на
файле, `rollback_to_sqlite` — единственная страховка переезда. Снятое раньше
времени превращает «осечка ничего не стоит» в «осечка стоит восстановления из
копии». Порядок такой: боевой переехал → неделя работы на MySQL → снятие.

### Что снимается целиком

| Где | Что | Почему одноразовое |
|---|---|---|
| `opencrm.sh` | `migrate_*` (десяток функций), `rollback_to_sqlite`, `cmd_migrate_mysql`, `MIGRATE_*`, пункт меню и разбор `migrate-mysql` | весь сценарий переезда |
| `opencrm.sh` | ветка SQLite в `choose_database`, `db_engine` | база станет одна, спрашивать нечего |
| `scripts/` | `migrate_to_mysql.py` целиком | переносить будет нечего |
| `docker/entrypoint.sh` | ветка `sqlite*` в снятии копии перед миграциями | останется только ветка MySQL |
| `docker/Dockerfile` | пакет `sqlite3` | нужен ровно копии SQLite и переезду |
| `deploy/updater.py` | `_snapshot` и `_restore_db` для файла, `db_file` | останутся `_snapshot_mysql` и `_restore_mysql` |
| `scripts/backup.sh`, `restore.sh`, `verify_backup.py` | ветки SQLite | обе базы там уже равноправны, снимается половина |
| `database/session.py` | `_prepare_sqlite` (WAL, `busy_timeout`, `foreign_keys=ON`), подмена `lower()`/`upper()` | всё это лечит особенности SQLite; в MySQL `LOWER()` знает Unicode сам |
| `web/api/routes/metrics.py` | ветка размера по файлу | останется запрос через `engine_info` |
| `docker-compose.yml` | профиль `mysql` у службы `db` | служба станет безусловной, как `redis` |
| тесты | `test_migrate_to_mysql.py`, части `test_mysql_setup.py`, `test_pre_migrate_snapshot.py` про SQLite | проверяют снимаемое |

### Что НЕ снимается, хотя выглядит связанным

- **`database/schema_check.py`** — сверка живой схемы с моделями от движка не
  зависит и остаётся правилом номер один.
- **`scripts/snapshot_db.py`** — это копия MySQL перед миграциями, она и есть
  новый стек.
- **`tests/test_mysql_portability.py`** — стережёт DDL, который SQLAlchemy
  соберёт для MySQL; живого сервера не требует и нужен вечно.
- **Проверка на сирот (`nayti_sirot`)** — переедет из `migrate_to_mysql.py` в
  отдельное место, если понадобится: она нужна не переезду, а любому разбору
  целостности. Пока переезд жив — живёт вместе с ним.
- **Redis** — он не половина переезда, а условие работы нескольких процессов.

### Чем проверять, что сняли ровно то, что хотели

1. `grep -rn "sqlite" --include=*.py --include=*.sh .` — в ответе не должно
   остаться ничего, кроме истории миграций и текстов документации.
2. Полный набор в обоих порядках и сборка образа гейта (`--target tests`):
   после снятия в нём станет меньше проверок, и это нормально — важно, чтобы
   пропущенных не прибавилось (стережёт `tests/test_gate_files.py`).
3. Установка с нуля на чистой машине: стек поднимается, `./opencrm.sh doctor`
   зелёный, `/healthz` отвечает `schema: ok`.
4. Обновление уже работающей установки: копия перед миграциями снимается
   дампом, `alembic upgrade head` проходит, откат возвращает базу из дампа.
