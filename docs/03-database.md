...
### Почему перенос идёт из копии

Сайт во время переезда работает, то есть живой файл меняется под руками. Сверка «строк
столько же» по такому источнику даёт ложную тревогу на честном переносе: пока шли данные,
кто-то завёл клиента. Копия неподвижна, поэтому сверка отвечает точно.

Цена названа вслух и печатается на экране: **записанное после снятия копии останется в
SQLite**, поэтому переезжать надо в тихую минуту. Живой файл при этом не открывается на
запись ни разу — копию снимает `sqlite3 .backup` (читатель), а скрипт переноса открывает
источник с `mode=ro&uri=true`. Испортить оригинал переезд не может ни при каком исходе.
...
Строковые поля не сверяются сознательно: движки по-разному отдают регистр и пробелы, и
сверка давала бы ложные тревоги вместо пользы.
