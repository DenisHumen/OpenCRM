...
Из сравнения выпадает ровно один вид столбцов — `Float`. Он компилируется в MySQL в
`FLOAT` (четыре байта против восьми у SQLite), и 0,3333333333 возвращается как 0,33333334
**на честном переносе тоже**: сверка краснела бы всегда и потому была бы выключена первым
же действием. Остальное сходится побайтно, и это свойство схемы, а не удача, — см.
