...
| `file_links` | ссылка на файл наружу: токен, режим (`view`/`download`), круг (`link`/`code`), срок, код, отзыв. Два внешних ключа — свой файл либо работа доски, — и ровно один заполнен: это проверка схемы (`CHECK`), а не запрос с замком, потому что правило про одну строку |
| `file_link_views` | одно открытие ссылки: когда, с какого адреса (хешем) и чем. Из них считаются «открывали N раз, M разных людей» — два числа, по которым решают, пора ли отзывать |
