...
| `documents`, `document_lines`, `document_events` | бланки, строки перечня и история состояний бланка. Перечень общий **у заказа, акта, накладной и возврата** (`LINE_KINDS` в `database/models/document.py`). Было сказано «у заказа и акта» — накладная взяла ту же таблицу с `b8d41f7a2c95`, потому что две параллельные системы строк это два места счёта денег, и расходятся они не сразу, а постепенно. У возврата своя колонка `refund_minor` — сколько вернули клиенту (`d4e7a1c9b2f6`, [22](../bloki/22-vozvraty.md)) |
| `document_files` | фото и видео к бумаге — сегодня только к возврату. По образцу `client_files`: файл на диске, в базе след; `document_id` CASCADE |
...
| `tasks` | напоминания: срок, исполнитель, необязательные клиент и заявка. `vazhnost` — одно из четырёх слов (`urgent`/`high`/`normal`/`low`, `b6e1f38c92d7`), второй ключ сортировки после «просрочено ли»; `note` — подробности карточки, `MEDIUMTEXT` и `deferred` (в списке из двухсот строк их не читают) |
| `task_files` | фото и видео к напоминанию. По образцу `document_files`: файл на диске, в базе след; `task_id` CASCADE, `uploaded_by` SET NULL |
| `file_folders` | папки блока «Файлы»: `parent_id` на себя (CASCADE), любая глубина. Ограничивать её значило бы решать за человека, как он раскладывает бумаги; от петли защищает служба обходом родителей, схема такого не умеет ([28](../bloki/28-fayly.md)) |
| `stored_files` | файлы, принесённые через блок «Файлы». Форма та же, что у вложений карточки: `file_uid` на диске, в базе след. `folder_id` NULL — корень «Загрузки»; папку снесли — файлы уходят с ней, а не всплывают в корень |
| `file_links` | ссылка на файл наружу: токен, режим (`view`/`download`), круг (`link`/`code`/`invited`), срок, код, отзыв. Два внешних ключа — свой файл либо работа доски, — и ровно один заполнен: это проверка схемы (`CHECK`), а не запрос с замком, потому что правило про одну строку |
| `file_link_views` | одно открытие ссылки: когда, с какого адреса (хешем), чем и — у круга «приглашённым» — под какой почтой. Из них считаются «открывали N раз, M разных людей» и журнал последних открытий, по которым решают, пора ли отзывать |
| `file_link_guests` | кого позвали по ссылке «только приглашённым»: адрес почты, приведённый к нижнему регистру. Пара «ссылка + адрес» уникальна: тот же адрес дважды — не ошибка, а «уже позвали», и различать «Ivan@X.ru» и «ivan@x.ru» значило бы поставить список в зависимость от того, как гость набрал собственное имя |
| `notifications` | уведомления сотруднику: вид, подстановки, ссылка, прочитано ли; текста нет — подпись собирает экран (docs/21 §4) |
...
| `deal_lines` | что продаём по заявке: товары, услуги из прайса и свои траты — одной таблицей. Решения ниже, разбор — [19-sborka-zakaza.md](../bloki/19-sborka-zakaza.md) |
| `snake_scores` | счёт со страницы обслуживания; адрес — хэшем, той же солью, что у просмотров витрины |
| `key_categories` | категории ключей двухфакторки. `zakrytaya` — внутрь пускают только создателя и root, и списком доступа её не открыть |
| `key_category_access`, `two_factor_key_access` | кому открыта категория и кому открыт отдельный ключ. Создателя и root в списках нет: им видно всегда, и строка о них была бы данными, которые можно снять |

## Пояснения к решениям

### two_factor_keys

- `secret_encrypted` — сам ключ сервиса, зашифрованный `core/security/secretbox`
  назначением `2fa-key-secret`. В открытом виде он не лежит нигде и не выходит
  из сервера: коды считаются на сервере, а сам ключ отдаётся только root в окне
  переноса на телефон, отдельной записью в журнале
  ([27-klyuchi.md](../bloki/27-klyuchi.md) §2).
- `backup_codes_encrypted` — запасные коды сервиса **списком в одном поле**, тем
  же шифром, но своим назначением `2fa-backup-codes`. Своей таблицей их не
  делали: они приходят и уходят пачкой, по одному их не ищут, а потраченность —
  признак внутри списка. `NULL` — их не заводили; пустой список означал бы
  «завели и все потратили», а это другое состояние.
- **Знак сервиса не хранится.** Он производное от `issuer` и вшитого набора
  значков, а производное в этой системе не хранят: сменится набор — знак обязан
  смениться сам.
- `task_id` — напоминание «сменить ключ» обычной задачей, `ON DELETE SET NULL`.
  Своего срока у ключа нет: он показывался бы и отсчитывался вторым способом и
  разошёлся бы с напоминаниями на первой правке.
- `deleted_at` — корзина. Секрет при этом остаётся зашифрованным на месте:
  «удалить» и «стереть» здесь разные слова, и второе необратимо.
- `vazhnost` — те же четыре слова, что у напоминаний (`urgent`/`high`/`normal`/
  `low`): срочный ключ получает ту же рябь по краю карточки, а вторая шкала
  важности означала бы объяснять человеку разницу, которой нет.


### users
- `role` — только `root` и `manager`, и это **не должность, а признак владельца системы**. Набор прав живёт в `role_id`; у root его нет и быть не может — права у него все и всегда, иначе можно было бы собрать конфигурацию, в которой раздать их обратно уже некому. Первый root создаётся скриптом bootstrap при первом запуске (email/пароль из env, `must_change_password = true`). Дальше root может сделать root'ом активного сотрудника, поэтому владельцев может быть несколько; система следит, чтобы хотя бы один остался (см. [07-bezopasnost.md](../ekspluatatsiya/07-bezopasnost.md)).
...
- `works.project_url` — ссылка на кейс клиента: каждая работа на витрине может быть отдельным проектом. Пустая строка = кнопки перехода нет. Пускаются только `http(s)` (ссылка уходит в `href` на публичной странице), см. [10-vitrina-keysy.md](../dizayn/10-vitrina-keysy.md).
- `works.preview_focus` — какой фрагмент работы попадает на витрину: 0 — верх картинки, 1 — низ. Форму места задаёт композиция, поэтому высота обрезки не хранится. `NULL` = от верха. Обрезана работа или нет — не хранится тоже: это производное от её сторон и формы её места, а место меняется от перестановки соседей (`layout.is_cropped`, см. [06-dizayn-vitriny.md](../dizayn/06-dizayn-vitriny.md)).
...
- `mail_accounts` — ящики фирмы, настройка уровня компании (правит обладатель `settings.manage`; у root оно есть всегда). `password_encrypted` — обратимо зашифрованный пароль (см. [07-bezopasnost.md](../ekspluatatsiya/07-bezopasnost.md)); `NULL` = пароль не задан, это не то же самое, что пустая строка. `last_sync_at = NULL` — ящик не читали ни разу: по нулю нельзя отличить новый ящик от сломанного. `last_error = NULL` — ошибок не было.
...
**Кадр поворачивается по метке EXIF до ужатия** (`media_service.povernut_po_metke`).
Телефон пишет снимок так, как лёг датчик, а то, как держали телефон, — отдельной
меткой Orientation. Браузер у JPEG её учитывает, но производные WEBP метку не
несут: без поворота снимок, снятый «вверх ногами» или боком, так и ложился на
диск (владелец 23.09.2026: «первое фото перевернулось, второе нормально»). Тот же
поворот стоит у аватара и у работ доски; у доски ещё и размер в метаданных
берётся после поворота (`razmer_po_metke`) — по нему считается рамка плитки.
Уже лежащие снимки не пересчитываются: оригинала товара на диске нет, и
перевёрнутый до правки снимок перезаливают.

Загрузка показывает ход (`api.zagruzka`): плитка-заглушка того же размера на месте
будущего снимка, проценты, «сколько из скольких» и отмена. Снимок с телефона —
это мегабайты, и без полосы на медленной связи не видно, идёт ли что-нибудь.
...
Разбор — [16-api-sayta.md](../ustroystvo/16-api-sayta.md) §10.

С 05.09.2026 рядом `api_key_hits` (миграция `c8d1f0a2b7e5`): строка на (ключ,
час, область) — `count` выполненных и `rejected` отказов по потолку;
`uq_api_key_hit` ведёт `api_key_id` и обслуживает внешний ключ (`CASCADE`:
ключ снят — статистика с ним), `ix_api_key_hits_bucket_at` — под уборку
старше 90 дней. Поштучный журнал не хранится — [16-api-sayta.md](../ustroystvo/16-api-sayta.md) §18.
...
что и само изменение. Что пишется и что намеренно не пишется — в [07-bezopasnost.md](../ekspluatatsiya/07-bezopasnost.md).
...
[17-nakladnye.md, §4.3](../bloki/17-nakladnye.md#43-чем-неизменяемость-обеспечена-технически);
...
Объём корзины (мягко удалённые клиенты) виден в карточке «Хранилище». Автоматика: cron с карантином 30 дней, см. [08-razvyortyvanie.md](../ekspluatatsiya/08-razvyortyvanie.md). Обзор всех медиафайлов досок с размером/датами и точечным удалением — менеджер файлов (`settings.manage`), см. [05-dizayn-crm.md](../dizayn/05-dizayn-crm.md).
...
## Отметка присутствия пишется перед самой фиксацией

`last_seen_at` ставится на КАЖДОМ обращении сотрудника — раз в минуту
(`PRESENCE_TOUCH_SECONDS`), но запросов в минуту у открытой CRM десятки. Писалась
она через ORM: `user.last_seen_at = now` и всё. А грязная строка уходит в базу на
первом же `autoflush`, то есть в НАЧАЛЕ запроса, и `UPDATE` запирает строку
сотрудника до конца транзакции — до конца запроса.

Браузер шлёт запросы пачкой, и все они от одного человека. Значит соседний
запрос упирался в строку, занятую предыдущим, ждал `innodb_lock_wait_timeout`
(полсотни секунд) и получал `(1205, 'Lock wait timeout exceeded')` и пятисотую.
Держал замок при этом не «долгий запрос» вообще, а любой сосед, успевший начать
раньше; на боевом сервере 21.09.2026 так падали `/live` и состояние хранилища.

Теперь `get_user_by_session` не пишет, а кладёт просьбу в `db.info`
(`auth_service.PRISUTSTVIE`), и `web/api/deps` исполняет её вплотную к `commit`.
Замок живёт микросекунды. Цена — в ответе ЭТОГО запроса `last_seen_at` ещё
прежний, отстаёт не больше минуты, а «в сети» считается по втрое большему порогу
(`ONLINE_THRESHOLD_SECONDS` — 150 с), то есть разницы не видно.

Стережёт дуэль `tests/test_prisutstvie_ne_zapiraet.py`: второе соединение с
потолком ожидания в секунду пишет в ту же строку, пока запрос работает. Вернёшь
запись через ORM — и оно получает ту самую `1205`.
...
**Раскладка сводки — `dashboard_json`** (06.09.2026): JSON со списком виджетов
(`kind`, `w`, `params`), у сотрудника, а не у фирмы. Пусто — умолчание экрана.
