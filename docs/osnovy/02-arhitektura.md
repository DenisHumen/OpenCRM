...
| Обработка изображений | **Pillow** | Превью, WebP-версии, blurhash-плейсхолдеры. Кадр с телефона сперва поворачивается по метке EXIF (`media_service.povernut_po_metke`): WEBP метку не несёт, и без поворота снимок ложился вверх ногами — см. `03-baza-dannyh.md`, снимки товара |
