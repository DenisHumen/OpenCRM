...
| Стили | **CSS-токены (custom properties)** | Ручная таблица стилей с токенами из дизайн-макетов — без сборочных зависимостей; палитра и компоненты 1:1 с `design/`. (В доке ранее значился Tailwind — заменён осознанно: макеты Claude Design дали готовую систему классов) |
...
(`scope="request"`) этот стек закрывается **после** отправки ответа. На SQLite
`commit` — запись в местный файл, она обычно успевает раньше, чем браузер
пришлёт следующий запрос. На MySQL это сетевой круг, и следующий запрос его
обгоняет.

Замерено (вход, сразу `GET /auth/me`, двадцать раз): SQLite — 1 промах из 20,
MySQL — **9 из 20**, с паузой в секунду — 0 из 20. На стенде с 2,7 млн строк —
